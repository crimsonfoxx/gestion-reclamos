...
namespace Projecto.Controllers
{
    public class HorariosController : Controller
    {
        ...
        {
            List<HorariosViewModel> lst;
            using (examenEntities db = new examenEntities())
            {

                lst = (from d in db.Horarios
                       select new HorariosViewModel
                       {
                           Id = d.Id,
                           Nombre = d.Nombre,
        ...
        {
            Tabla5ViewModel model = new Tabla5ViewModel();
            using (examenEntities db = new examenEntities())
            {
                var oTabla = db.Horarios.Find(Id);
        ...
        [HttpPost]
        public ActionResult Editar(Tabla5ViewModel model)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    using (examenEntities db = new examenEntities())
                    {
                        var oTabla = db.Horarios.Find(model.Id);
